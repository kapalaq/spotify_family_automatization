...
from SpotifyBot.app.tools import LinkForm, SettingsForm
...
async def update_cmd(message: types.Message):
    ...
    await admin.update_settings(message)


@admin_router.message(Command("update"))
async def update_settings(message: types.Message, state: FSMContext):
    """Update settings values.

    This function allows admin to update settings,
    such as bank accounts or subscription price.
    ...
    Returns:
        None, will start dialogue on /update successful validation.
...
@admin_router.message(SettingsForm.action)
async def process_payment_at(message: types.Message, state: FSMContext):
...
async def confirm_link_form(message: types.Message, state: FSMContext):
